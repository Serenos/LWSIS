...
from .dynamic_mask_head import build_dynamic_mask_head, get_point_coords_wrt_img, point_sample
...
def get_lidar_dis_similarity2(lidar_in, lidar_out, stride):
    '''
    lidar_in: tensor [N, 6], x,y,z,i,u,v
    '''
    N_in = lidar_in[:, 0].nonzero().shape[0]
    N_out = lidar_out[:, 0].nonzero().shape[0]
    N_max = lidar_in[:, 0].shape[0] + lidar_out[:, 0].shape[0]
    pc_max = torch.zeros_like(torch.cat((lidar_in, lidar_out), dim=0))
    pc_all = torch.cat((lidar_in[:N_in, :], lidar_out[:N_out, :]), dim=0)
    pc_max[:N_in+N_out, :] = pc_all
    diff = EuclideanDistance(pc_max[:, :3], pc_max[:, :3])
    weight = torch.zeros_like(diff)
    sim = torch.zeros_like(diff)
    if (N_in+N_out)!= 0 and (N_in+N_out)!=1:
        sorted_diff, _ = torch.sort(diff[:N_in+N_out,:N_in+N_out], dim=1)
        min_dis = torch.median(sorted_diff[:, 1])
        sim = torch.exp(-(diff/min_dis-1) * 0.5)
        weight[:N_in+N_out, :N_in+N_out] = 1
    
    #for visualize
    # import matplotlib.pyplot as plt
    # sim_vis = sim.to('cpu').numpy()
    # weight_vis = weight.to('cpu').numpy()
    # fig, ax = plt.subplots(1, 1, figsize=(20, 20))
    # ax.imshow((sim_vis>0.5)*weight_vis)
    # plt.show() 
    
    return sim * weight, pc_max[:, -2:]/stride

def compute_lidar_color_sim(image, coords):
    N = coords[:, 0].nonzero().shape[0]
    with torch.no_grad():
        coords_wrt_img = coords.clone()
        coords_wrt_img[:,  0] /= image.shape[1]
        coords_wrt_img[:, 1] /= image.shape[0]

    image = image.to(torch.float64).unsqueeze(0).unsqueeze(0)
    coords_wrt_img = coords_wrt_img.unsqueeze(0).unsqueeze(0)
    lidar_color = point_sample(
        image,
        coords_wrt_img,
        align_corners=False,
    )
    lidar_color = lidar_color[0,0,0,:].unsqueeze(1)

    diff = EuclideanDistance(lidar_color, lidar_color)
    sim = torch.exp(-diff * 0.5)
    weight = torch.zeros_like(diff)
    if N!=0 and N!=1:
        weight[:N,:N] = 1
    #for visualize
    ...
    # weight_vis = weight.to('cpu').numpy()
    # fig, ax = plt.subplots(1, 1, figsize=(20, 20))
    # ax.imshow((sim_vis>0.3)*weight_vis)
    # plt.show() 
    return sim * weight

def compute_resnet_sim(resnet_feat, coords):
    N = coords[:, 0].nonzero().shape[0]
    with torch.no_grad():
        coords_wrt_img = coords.clone() / 2
        coords_wrt_img[:,  0] /= resnet_feat.shape[2]
        coords_wrt_img[:, 1] /= resnet_feat.shape[1]

    resnet_feat = resnet_feat.to(torch.float64).unsqueeze(0)
    coords_wrt_img = coords_wrt_img.unsqueeze(0).unsqueeze(0)
    lidar_resnet_feat = point_sample(
        resnet_feat,
        coords_wrt_img,
        align_corners=False,
    )
    lidar_resnet_feat = lidar_resnet_feat[0,:,0,:]

    #cosin sim
    sim = torch.mm(lidar_resnet_feat.T, lidar_resnet_feat)
    sim_norm = sim / (sim.max() - sim.min() + 1e-5)
    #gaussion sim
    # diff = EuclideanDistance(lidar_resnet_feat.T, lidar_resnet_feat.T)
    # sim_norm = torch.exp(-diff * 0.5)

    weight = torch.zeros_like(sim_norm)
    if N!=0 and N!=1:
        weight[:N,:N] = 1
    return sim_norm * weight
...
class CondInst(nn.Module): 
    ...
    def __init__(self, cfg):
        ...
        self.use3dpoint = cfg.INPUT.USE_3DPOINTS
    ...
    def forward(self, batched_inputs):
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.boxinst_enabled:
                ...
                if self.use3dpoint:
                    resnet_feat = features['p3'].detach()
                    self.add_point_from_lidar(gt_instances, resnet_feat)
                    #for visualize
                    #self.visualize_lidar_sim(original_images.tensor, gt_instances)
            else:
    ...
    def add_point_from_lidar(self, instances, resnet_feat):
        stride = self.mask_out_stride

        for im_i, per_im_gt_inst in enumerate(instances):
            per_im_boxes = per_im_gt_inst.gt_boxes.tensor
            per_im_lidarsim, per_im_resnet_sim = [], []
            per_im_lidar_coord = []
            #image_lab = per_im_gt_inst.image_lab[0]
            for i, per_box in enumerate(per_im_boxes):
                lidar_sim, lidar_coord = \
                    get_lidar_dis_similarity2(per_im_gt_inst.gt_lidar_in[i], per_im_gt_inst.gt_lidar_out[i], stride)

                per_im_lidarsim.append(lidar_sim)
                per_im_lidar_coord.append(lidar_coord)

                # lidar_color_sim = compute_lidar_color_sim(image_lab, lidar_coord)
                # per_im_lidar_rgb_sim.append(lidar_color_sim)
                resnet_sim = compute_resnet_sim(resnet_feat[im_i], lidar_coord)
                per_im_resnet_sim.append(resnet_sim)

            per_im_gt_inst.gt_lidarsim = torch.stack(per_im_lidarsim, dim=0)
            per_im_gt_inst.gt_lidarcoord = torch.stack(per_im_lidar_coord, dim=0)
            #per_im_gt_inst.gt_lidar_rgb_sim = torch.stack(per_im_lidar_rgb_sim, dim=0)
            per_im_gt_inst.gt_resnet_sim = torch.stack(per_im_resnet_sim, dim=0)
            #for visualize lidarpoint
            # for i in range(len(per_im_gt_inst)):
            #     mask0 = per_im_gt_inst.get("gt_bitmasks")[i].to('cpu').numpy()
            #     #point_coords = per_im_gt_inst.get("gt_point_coords")[i].to('cpu').numpy()
            #     #point_labels = per_im_gt_inst.get("gt_point_labels")[i].to('cpu').numpy()
            #     lidar_coord = per_im_gt_inst.get("gt_lidarcoord")[i].to('cpu').numpy()
            #     fig, ax = plt.subplots(1, 1, figsize=(20, 20))
            #     ax.imshow(mask0)
            #     ax.scatter(lidar_coord[:, 0], lidar_coord[:, 1], s=10, c='r')
            #     plt.show()
    ...
    def visualize_lidar_sim(self, images, instances):
        import matplotlib.pyplot as plt
        import os
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            lidar_coords = per_im_gt_inst.gt_lidarcoord.to('cpu').numpy()
            lidarsim = per_im_gt_inst.gt_lidarsim.to('cpu').numpy()
            point_coords = per_im_gt_inst.gt_point_coords.to('cpu').numpy()
            point_labels = per_im_gt_inst.gt_point_labels.to('cpu').numpy()
            img = downsampled_images[im_i].to('cpu').numpy().transpose(1,2,0)/255
            import os
            save_path = os.path.join(os.environ['HOME'], 'picture/lidar_sim/')
            os.makedirs(save_path, exist_ok=1)
            
            #for i in range(len(per_im_gt_inst)):
            i=2
            coord, sim = lidar_coords[i], lidarsim[i]
            point_coord, point_label = point_coords[i], point_labels[i]

            fig, ax = plt.subplots(1, 1, figsize=(20, 20))
            ax.scatter(point_coord[:,0], point_coord[:,1], s=10, c=point_label)
            ax.imshow(img)
            ax.axis('off')
            plt.show()
            plt.savefig(save_path+'point_label.jpg')

            fig, ax = plt.subplots(1, 1, figsize=(20, 20))
            ax.scatter(coord[:,0], coord[:,1], s=5, c='r')
            ax.imshow(img)
            ax.axis('off')
            plt.show()
            plt.savefig(save_path+'lidar_point.jpg')

            N = coord.nonzero()[0][-1]
            fig, ax = plt.subplots(1, 1, figsize=(20, 20))
            for i in range(N):
                
                ax.scatter(coord[:,0], coord[:,1], s=5, c=sim[i]>0.5)
                ax.scatter(coord[i,0], coord[i,1], s=15, c='r')
                #ax.scatter(point_coord[:,0], point_coord[:,1], s=10, c=point_label)
                ax.imshow(img)
                ax.axis('off')
                plt.show() 
